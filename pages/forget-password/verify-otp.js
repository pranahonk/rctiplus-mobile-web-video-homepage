...
class VerifyOtp extends React.Component {
    ...
    componentDidMount() {
        ...
        this.setState({ username, token }, () => {
            this.props.getOtp(this.state.username, 'forget-password',this.isPhoneCode(phone_code), token)
                .then(response => {
    ...
    showAlert() {
		...
		showConfirmAlert(this.state.alert_message, 'OTP Limits', () => {
            this.props.getOtp(username, 'forget-password',  this.isPhoneCode(phone_code), token)
                .then(response => {
                    let newState = {};
