...
class Trending_v2 extends React.Component {
  ...
  static async getInitialProps(ctx) {
    ...
    if (isEmpty(ctx.query)) {
      metaSeo = [{
        title: SITEMAP.trending.title,
        description: SITEMAP.trending.description,
        keyword: SITEMAP.trending.keywords,
      }];
    } else {
      ...
      if (this.isInViewport(document.getElementById(eventId))) {
        ...
        if (eventId) {
          ...
          googletag.cmd.push(function() {
            googletag.display('div-gpt-ad-1591240670591-0');
          });
          ` }}>
                </script>
                    </div>
      ...
      <Layout title={this.props?.metaSeo?.title}>
        <Head>
          ...
          <meta property='og:image' itemProp='image'
                content={this.props.metaOg?.cover || this.props?.general?.img_logo} />
        ...
        <div
          className={`main-content ${this.state.sticky_category_shown ? 'sticky-menu-category-active' : ''} ${(this.platform === 'ios' || this.platform === 'android') && 'apps-mode'}`}
          style={{ marginTop: this.platform === 'ios' ? 26 : this.platform === 'android' ? 15 : '110px' }}>
          {this.state.load_error ? (
            ...
            this.state.load_content_error ? (
              <div style={{
                ...
                <p style={{ fontSize: 12, fontWeight: 300 }}>Please try again later, we're working to fix the
                  problem</p>
              </div>
            ) : (
              <div>
                  ...
                  <StickyContainer>
                    <Sticky>
                      {({ distanceFromTop }) => {
                        ...
                        return (
                          <div
                            className={`navigation-container ${sticky_category_shown ? 'sticky-menu-category' : 'sticky-menu-inactive'}`}>
                            <Nav tabs className='navigation-tabs'>
                              {this.state.tabs.map((tab, i) => (
                                <NavItem
                                  ...
                                  <NavLink onClick={() => this.toggleTab(tab.id.toString(), tab)}
                                           className='item-link'>{tab.name}</NavLink>
                                </NavItem>
                              ))}
                            </Nav>
                              ...
                              <AddIcon onClick={() => {
                                ...
                                if (true) {
                                  ...
                                  showSignInAlert(`Please <b>Sign In</b><br/>
                                                                            ...
                                                                            <b>RCTI+</b>`, '', () => {
                                  }, true, 'Sign Up', 'Sign In', true, true);
                                }
                              }} />
                            </div>
                          </div>
                    ...
                    return (
                      <TabPane key={i} tabId={tab.id.toString()}>
                        {(this.state.is_trending_loading ? (<HeadlineLoader />) : (
                          !isEmpty(this.state.trending_articles[tab.id]) ?
                            <HeadlineCarousel className='news-carousel' articles={this.state.trending_articles[tab.id]}
                                              assets_url={this.state.assets_url} /> : null
                        ))}
                        {!isEmpty(this.props.newsv2.data_topic) ? (
                          <div className='interest-topic_wrapper'>
                            <div className='interest-topic_title'>
                              <h1>topik menarik</h1>
                              <Link
                                href={`/news/interest-topic${this.accessToken ? `?token=${this.accessToken}&platform=${this.platform}` : ''}`}
                                className=''><span className='news-more_action'>See More <ArrowForwardIosIcon /></span></Link>
                            </div>
                                  ...
                                  return i > 10 ? ''
                                    : (
                                      <Col xs='6' className='interest-topic-item' key={i}>
                                        <Link
                                          href={`/news/topic/tag/${item.tag.toLowerCase()}${this.accessToken ? `?token=${this.accessToken}&platform=${this.platform}` : ''}`}>
                                          {`#${item.tag}`}
                                        </Link>
                                      </Col>
                                    );
                            ...
                            return (((j + 1) % 7 === 0) ?
                                (<li className='listItems' key={j + article.title}>
                                  <ListGroup className='groupNews'>
                                    <ListGroupItem
                                      className={`listNewsAdds ${!this.state.is_ads_rendered ? 'blank-space' : ''}`}>
                                      <iframe
                                        onLoad={() => {
                                          ...
                                          <div className='article-description'>
                                            ...
                                            <div className='article-title-container'>
                                              <h4 className='article-title'
                                                  dangerouslySetInnerHTML={{ __html: article.title.replace(/\\/g, '') }}></h4>
                                              <p style={{ display: 'none' }}
                                                 dangerouslySetInnerHTML={{ __html: readMore(article.content) }}></p>
