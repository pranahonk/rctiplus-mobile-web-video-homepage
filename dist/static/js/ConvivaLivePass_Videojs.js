...
// namespace
    ...
    function ConvivaContentInfo() {
        ...
        // DE-4842 :: removed defaultBitrate reporting also now  if externalBitrateReporting is true then no autodetection even not initially
                //untill setBitrate api is called first time, because this change two different variables is not required so removing externalBitrateSet
...
function LivePass() {
    ...
    var parseUrl = function (url) {
        if(Lang.isValidString(url)) {
            var parser = url.split("://");
            if (parser[1] != "cws.conviva.com" && (parser[0] == "https" || parser[0] == "http")) {
                // Return gatewayUrl set by application for any https ot http url without cws.conviva.com
                return url;
            }
        }
        return ("https://" + LivePass._settings.customerKey+ "." + "cws.conviva.com");
    }
...
// namespace monitor
    ...
    function Monitor() {
        ...
        function __SetStream(targetStream /* : StreamInfo */) /* : void */  {
            ...
            if (targetStream.bitrateKbps <= -2 || _s._externalBitrateReporting) targetStream.bitrateKbps = _s._streamInfo.bitrateKbps;
        ...
        function __setBitrate(newBitrateKbps /* : int */) /* : void */  {
            if (_s._externalBitrateReporting && _s._streamer) {
        ...
        function __updateHeartbeat(heartbeat /* : DictionaryCS(string, object) */) /* : void */  {
            ...
            if (_s._streamInfo.GetBitrateKbps() > 0) {
                _s._utils.logSession('_s._externalBitrateReporting :: ',+_s._externalBitrateReporting,_s._sessionId);
        ...
        function __enqueueStreamChangeEvent(oldStream /* : StreamInfo */, newStream /* : StreamInfo */) /* : void */  {
            ...
            if (oldStream.GetBitrateKbps() != newStream.GetBitrateKbps() && newStream.GetBitrateKbps() > 0) {
                //DE-4837 : Provide a way to identify if bitrate is auto-detected or set using API.
                if(_s._externalBitrateReporting === true){
                    _s._utils.logSession('Bitrate change requested from the application',_s._sessionId);
                }else{
                    _s._utils.logSession('Bitrate change requested from the Conviva Library', _s._sessionId);
                }
...
// namespace session
    function Session() {
        ...
        if(_s != STAT_INIT) _s._heartbeatIntervalMs /* : int */ = 0;
        if(_s != STAT_INIT) _s._gatewayUrl /* : String */ = 0;
        if(_s != STAT_INIT) _s._heartbeatSendingTime /* : int */ = 0;
        if(_s != STAT_INIT) _s._heartbeatReceivedTime /* : int */ = 0;
        if(_s != STAT_INIT) _s._hbInfos /* : Array */ = [];
        if(_s != STAT_INIT) _s._maxhbinfos /* : String */ = 2;
        ...
        function _constr(streamer /* : Object */, contentInfo /* : ConvivaContentInfo */, options /* : Object */, global /* : Boolean */, networkInfo /* NetworkInfo */) {
            ...
            _s._heartbeatIntervalMs = _s._settings.heartbeatIntervalMs;
            _s._gatewayUrl = _s._settings.gatewayUrl;
        ...
        function __resetHeartbeatTimer() /* : void */  {
            ...
            _s._heartbeatTimer = _s._utils.createTimer(_s.sendHeartbeat, _s._heartbeatIntervalMs, "sendHeartbeat");
        }
        ...
        function __makeHeartbeat() /* : Object */  {
            ...
            //MR: DE-5156 : adding hbinfo in to the heartbeat from seq number 1
            // and if status is pending sending RTT as -1 for all the entries
            if(_s._heartbeatSequenceNumber > 0){
                _s._hbInfos.forEach(element => {
                    if(element.err === 'pending'){
                        element.rtt = -1;
                    }
                });
         ...
         */
        //MR: DE-5156: start point of RTT calculation
        ...
        function __encodeAndPostHeartbeat(heartbeat /* : Object */, afterPostCbk /* : Function */) /* : void */  {
            ...
            var url /* : String */ = _s._gatewayUrl + _s._settings.gatewayPath;
            var contentType /* : String */ = "application/json";
            var seq = _s._heartbeatSequenceNumber-1;
            //MR: DE-5156 : RTT calculation : heart beat sent time
            _s._heartbeatSendingTime = new Date().getTime();
            _s._utils.httpRequest(true, url, jsonHeartbeat, contentType, _s.onHeartbeatResponse);
            //MR: DE-5156: RTT calculation : pushing hbinfo into hbinfos array with 
            //satus as pending and then maintaining the max count in the array.
            _s._hbInfos.push({'err':'pending','seq' : seq,'rtt': _s._heartbeatSendingTime});
            if(_s._hbInfos.length > _s._maxhbinfos){
                _s._hbInfos.shift();
            }
         ...
         //MR: DE-5156: End point of RTT calculation
        ...
        function __onHeartbeatResponse(isSuccess /* : Boolean */, jsonResponse /* : String */) /* : void */  {
            ...
            if (_s._utils != null) {
                _s._utils.runProtected(
                    function () {
                        //MR: DE-5156: calculate RTT : updating the actual RTT time for the hb response item
                        //and the deleting all the previous entries from the hbinfos array.
                        _s._heartbeatReceivedTime = new Date().getTime();
                        var decodedResponse /* : DictionaryCS(string, object) */ = _s._utils.jsonDecode(jsonResponse);
                        var seq = decodedResponse.GetValue('seq');
                        var sliceIndex = -1;
                        _s._hbInfos.some((element,index) => {
                            if(element.seq === seq){
                                element.rtt = _s._heartbeatReceivedTime - element.rtt;
                                element.err = 'ok';
                                sliceIndex = index;
                                return true;
                            }
                        });
                        if(sliceIndex && sliceIndex > -1 && _s._hbInfos.length > 1){
                            _s._hbInfos.splice(0,sliceIndex);
                        }
                        if (isSuccess) {
                            if (decodedResponse != null) {
                                ...
                                if(cfg){
                                    ...
                                    if (cfg.hbi) {
                                        ...
                                        if(1000 * heartbeatIntervalSec !=  _s._heartbeatIntervalMs) {
                                             _s.log("Received hbInterval from server " + heartbeatIntervalSec);
                                             _s._heartbeatIntervalMs = 1000 * heartbeatIntervalSec;
                                             if (_s._heartbeatTimer != null) {
                                                  _s.resetHeartbeatTimer();
                                             }
                                        }
                                    ...
                                    if (cfg.gw) {
                                        ...
                                        if(gatewayUrl != _s._gatewayUrl) {
                                            _s.log("Received gatewayUrl from server "+gatewayUrl);
                                            _s._gatewayUrl = gatewayUrl;
                                        }
                                    }
                                    if (cfg.maxhbinfos) {
                                        if(_s._maxhbinfos !== cfg.maxhbinfos){
                                            _s.log("Received maxhbinfos from server " + cfg.maxhbinfos);
                                        }
                                        _s._maxhbinfos = cfg.maxhbinfos;
                                    }
                                }
...
// namespace utils
    function Settings() {
        ...
        if(_s != STAT_INIT) _s.clientVersion /* : String */ = "2.151.0.36991";
...
function ConvivaVideojsStreamerProxy() {
  ...
  function _constr(vjsPlayer) {
    ...
    if (_s.tech.hls) {
      ...
      var updatedStream = new StreamInfo(bandW, null, -1, -1, -1);
      _s.SetStream(updatedStream);
  ...
  this.updateBitrate = function() {
    ...
    if(_s._vjsPlayer && _s._vjsPlayer.dash) {
      ...
      if(totalBitrate) {
          var updatedStream = new StreamInfo(Math.round(totalBitrate / 1024), null, -1, -1, -1);
          _s.SetStream(updatedStream);
      }
    }
  }
  ...
  this._poll = function() {
     Utils.getInstance().runProtected(function() {
         ...
         // CSR-1779 : Check availability of API's before accessing it
         if(typeof _s._vjsPlayer.currentTime === 'function') {
             _s.SetPlayheadTimeMs(_s._vjsPlayer.currentTime());
         }

         if(typeof _s._vjsPlayer.duration === 'function') {
             _s.SetDuration(_s._vjsPlayer.duration());
         }

         if(typeof _s._vjsPlayer.videoWidth === 'function' && typeof _s._vjsPlayer.videoHeight === 'function') {
  ...
  this.onProgress = function() {
    Utils.getInstance().runProtected(function() {
      ...
      if (_s._vjsPlayer && _s._vjsPlayer.tech(_s.requestTech).hls && _s._vjsPlayer.tech(_s.requestTech).hls.playlists.media() && _s._vjsPlayer.tech(_s.requestTech).hls.playlists.media().attributes) {
        var updatedStream = new StreamInfo(Math.round(_s._vjsPlayer.tech(_s.requestTech).hls.playlists.media().attributes.BANDWIDTH / 1000), null, -1, -1, -1);
        _s.SetStream(updatedStream);
      } else {
        _s.updateBitrate();
      }
  ...
  this._convertError = function(code) {
    // CSR-1795 : Declaring variable before acessing
    var errStr = 'MEDIA_ERR_UNKNOWN';
