...
export default function NewsDetailContent({item, indexKey, isIndexKey}) {
  ...
  const rmAttributes = item.content.replace(/(class|id|style)="\w+"/gm, '').replace(/\s*>/gmi, '>').replace(/(<!--\s*([a-zA-Z0-9_ ]*)\s*-->)/gm, '').replace(/\\|\'/gm, '').replaceAll(/https?:\/\/(www\.)?rctiplus\.com\//gm, 'https://m.rctiplus.com/');
  const countTag = {};
