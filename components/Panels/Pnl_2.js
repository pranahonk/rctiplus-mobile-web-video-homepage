	...
	link(data) {
		...
		switch (data.content_type) {
			...
			default:
				...
				Router.push(`/programs/${data.program_id}/${data.program_title.replace(/ +/g, '-').replace(/#+/g, '').toLowerCase()}/${data.content_type}/${data.content_id}/${data.content_title.replace(' ', '-').toLowerCase()}?ref=homepage&homepage_title=${this.props.title}`);
				break;
		}
	}
